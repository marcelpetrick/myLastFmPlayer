...
Plan: Fix background auto-download for cached and freshly-fetched tracks
 ...
 - Skips already-resolved tracks (QUEUED) with no yt-dlp calls — O(n) in-memory pass
     ...
     ├────────────────────────────────┼────────────────────────────────────────────────────────────────────────────────────────────────────
     ┤
     │                                │ __init__ (+1 flag), fetch_loved_tracks (~376–384), _handle_tracks_loaded (~655–685),           
     │
     │ my_lastfm_player/controller.py │ _start_automatic_download (~955–961), _handle_tracks_resolved (~773–814),                      
     │
     │                                │ _handle_tracks_downloaded (~816–851)
     │
     └────────────────────────────────┴────────────────────────────────────────────────────────────────────────────────────────────────────
     ┘
     ...
     1. Cold start (no cache): run app, enter username, click Fetch. Verify lookup + download start automatically without pressing
     Download.
     2. Warm start (cache exists): run app, enter username, click Fetch. Verify download starts immediately (lookup is fast, no yt-dlp
     calls for resolved tracks).
     3. Concurrency: confirm only one DownloadTracksWorker is active at a time (log line "Worker started downloading" appears once per run,
      not twice).
     4. Run ./localPipeline.sh — all tests must pass before commit.
